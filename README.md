...
The CDN (`sdk.yaplet.com`, Cloudflare Pages) is served from the committed
`published/` directory. After building:

```bash
npm run deploy   # triggers the Cloudflare Pages deploy hook
```

`build/` is **not** committed (it is regenerated by `prepare` on install and by
`npm run build`). Only `published/` is committed and served.
