...
const Tours = (function () {
	...
	function createOverlaySvg(stage) {
		...
		svg.classList.add("yaplet-tour-overlay", "yaplet-tour-overlay-animated");
	...
	function mountDummyElement() {
		const existingDummy = document.getElementById("yaplet-tour-dummy-element");
		if (existingDummy) {
			return existingDummy;
		}
		let element = document.createElement("div");
		element.id = "yaplet-tour-dummy-element";
	...
	function refreshActiveHighlight() {
		...
		// Re-resolve the best visible element from fallback selectors on resize
		if (activeStep && activeStep.fallbackSelectors) {
			const bestElement = resolveElement(activeStep);
			if (bestElement && bestElement !== activeHighlight) {
				transferHighlight(bestElement, activeStep);
				return;
			}
		}
	...
	function transferHighlight(toElement, toStep) {
		...
		const isToDummyElement = toElement.id === "yaplet-tour-dummy-element";
		const isFromDummyElement = fromElement.id === "yaplet-tour-dummy-element";
		...
		fromElement.classList.remove(
			"yaplet-tour-active-element",
			"yaplet-tour-no-interaction"
		);
		...
		if (disableActiveInteraction) {
			toElement.classList.add("yaplet-tour-no-interaction");
		}
		toElement.classList.add("yaplet-tour-active-element");
		toElement.setAttribute("aria-haspopup", "dialog");
		toElement.setAttribute("aria-expanded", "true");
		toElement.setAttribute("aria-controls", "yaplet-tour-popover-content");
	}
	function destroyHighlight() {
		var _a;
		(_a = document.getElementById("yaplet-tour-dummy-element")) == null
			? void 0
			: _a.remove();
		document
			.querySelectorAll(".yaplet-tour-active-element")
			.forEach((element) => {
	...
	function renderPopover(element, step) {
		...
		if (
			...
			popover.nextButton.classList.add("yaplet-tour-popover-btn-disabled");
		}
		if (
			disabledButtonsConfig == null
				? void 0
			...
			popover.previousButton.classList.add("yaplet-tour-popover-btn-disabled");
		}
		if (
			disabledButtonsConfig == null
				? void 0
			...
			popover.closeButton.classList.add("yaplet-tour-popover-btn-disabled");
		}
		...
		onDriverClick(
			...
			(e) => {
				...
				if (target.classList.contains("yaplet-tour-popover-next-btn")) {
					if (onNextClick) {
				...
				if (target.classList.contains("yaplet-tour-popover-prev-btn")) {
					if (onPrevClick) {
				...
				if (target.classList.contains("yaplet-tour-popover-close-btn")) {
					if (onCloseClick) {
			...
			(target) => {
				return (
					...
					target.className.includes("yaplet-tour-popover")
				);
			}
		);
	...
	function repositionPopover(element, step) {
		...
		const requiredSide =
			element.id === "yaplet-tour-dummy-element" ? "over" : side;
		...
		if (!noneOptimal) {
			...
			popover.arrow.classList.add("yaplet-tour-popover-arrow-none");
		}
	}
	function renderPopoverArrow(alignment, side, element) {
		...
		if (!arrowSide) {
			popoverArrow.classList.add("yaplet-tour-popover-arrow-none");
		} else {
			popoverArrow.classList.add(`yaplet-tour-popover-arrow-side-${arrowSide}`);
			popoverArrow.classList.add(
				`yaplet-tour-popover-arrow-align-${arrowAlignment}`
			);
		}
	}
	function createPopover() {
		const wrapper = document.createElement("div");
		wrapper.classList.add("yaplet-tour-popover");
		const arrow = document.createElement("div");
		arrow.classList.add("yaplet-tour-popover-arrow");
		const title = document.createElement("header");
		title.id = "yaplet-tour-popover-title";
		title.classList.add("yaplet-tour-popover-title");
		title.style.display = "none";
		title.innerText = "Popover Title";
		const description = document.createElement("div");
		description.id = "yaplet-tour-popover-description";
		description.classList.add("yaplet-tour-popover-description");
		description.style.display = "none";
		description.innerText = "Popover description is here";
		const closeButton = document.createElement("button");
		closeButton.type = "button";
		closeButton.classList.add("yaplet-tour-popover-close-btn");
		closeButton.setAttribute("aria-label", "Close");
		closeButton.innerHTML = "&times;";
		const footer = document.createElement("footer");
		footer.classList.add("yaplet-tour-popover-footer");
		const progress = document.createElement("span");
		progress.classList.add("yaplet-tour-popover-progress-text");
		progress.innerText = "";
		const footerButtons = document.createElement("span");
		footerButtons.classList.add("yaplet-tour-popover-navigation-btns");
		const previousButton = document.createElement("button");
		previousButton.type = "button";
		previousButton.classList.add("yaplet-tour-popover-prev-btn");
		previousButton.innerHTML = "&larr; Previous";
		const nextButton = document.createElement("button");
		nextButton.type = "button";
		nextButton.classList.add("yaplet-tour-popover-next-btn");
	...
	function driver(options = {}) {
		...
		function init() {
			...
			document.body.classList.add(
				"yaplet-tour-active",
				getConfig("animate") ? "yaplet-tour-fade" : "yaplet-tour-simple"
			);
			initEvents();
		...
		function destroy(withOnDestroyStartedHook = true) {
			...
			if (activeElement && activeStep) {
				const isActiveDummyElement =
					activeElement.id === "yaplet-tour-dummy-element";
				if (onDeselected) {
					onDeselected(
						isActiveDummyElement ? void 0 : activeElement,
						activeStep,
